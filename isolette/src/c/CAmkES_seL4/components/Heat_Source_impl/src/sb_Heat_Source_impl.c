...
Unit isolette_Devices_Heat_Source_impl_seL4Nix_heat_out_Send(
  STACK_FRAME
  art_DataContent d) {
  DeclNewStackFrame(caller, "sb_Heat_Source_impl.c", "", "isolette_Devices_Heat_Source_impl_seL4Nix_heat_out_Send", 0);
  // FIXME: dummy implementation
}

// is_empty heat_control: In DataPort
B isolette_Devices_Heat_Source_impl_seL4Nix_heat_control_IsEmpty(STACK_FRAME_ONLY) {
  return sb_heat_control_is_empty();
}

// receive heat_control: In DataPort union_art_DataContent
Unit isolette_Devices_Heat_Source_impl_seL4Nix_heat_control_Receive(
  STACK_FRAME
  Option_8E9F45 result) {
  DeclNewStackFrame(caller, "sb_Heat_Source_impl.c", "", "isolette_Devices_Heat_Source_impl_seL4Nix_heat_control_Receive", 0);
...
void pre_init(void) {
  ...
  printf("Entering pre-init of Heat_Source_impl\n");
  ...
  isolette_Devices_Heat_Source_impl_adapter_initialiseArchitecture(SF_LAST);

  // call the component's initialise entrypoint
  isolette_Devices_Heat_Source_impl_adapter_initialiseEntryPoint(SF_LAST);

  printf("Leaving pre-init of Heat_Source_impl\n");
}
...
int run(void) {
  ...
  for(;;) {
    ...
    isolette_Devices_Heat_Source_impl_adapter_computeEntryPoint(SF_LAST);
    sb_self_pacer_tick_emit();
  }
  return 0;
}
