...
Unit building_control_gen_mixed_SharedMemory_receive(STACK_FRAME Z port, MBox2_1CBFC4 out) {
...
Unit building_control_gen_mixed_SharedMemory_receiveAsync(STACK_FRAME Z port, MBox2_1029D1 out) {
    int sid = semget((key_t) port, 1, 0666);
...
Unit building_control_gen_mixed_SharedMemory_send(STACK_FRAME Z destid, Z port, art_DataContent d) {
    int sid = semget((key_t) port, 1, 0666);
...
B building_control_gen_mixed_SharedMemory_sendAsync(STACK_FRAME Z destid, Z port, art_DataContent d) {
    int sid = semget((key_t) port, 1, 0666);
