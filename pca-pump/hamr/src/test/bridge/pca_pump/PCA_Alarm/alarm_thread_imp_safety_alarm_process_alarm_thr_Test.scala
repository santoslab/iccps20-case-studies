...
class alarm_thread_imp_safety_alarm_process_alarm_thr_Test extends BridgeTestSuite[alarm_thread_imp_safety_alarm_process_alarm_thr_Bridge](Arch.pca_imp_Instance_safety_alarm_process_alarm_thr) {
  ...
  // getter for out EventDataPort
  def get_Log_Fault(): Option[PCA_Types.Fault_Record] = {
    val value: Option[PCA_Types.Fault_Record] = get_Log_Fault_payload() match {
      case Some(PCA_Types.Fault_Record_Payload(v)) => Some(v)
      case Some(v) => fail(s"Unexpected payload on port Log_Fault.  Expecting 'PCA_Types.Fault_Record_Payload' but received ${v}")
      case _ => None[PCA_Types.Fault_Record]()
    }
    return value
  }

  // payload getter for out EventDataPort
  def get_Log_Fault_payload(): Option[PCA_Types.Fault_Record_Payload] = {
    return ArtNative_Ext.observeOutPortValue(bridge.initialization_api.Log_Fault_Id).asInstanceOf[Option[PCA_Types.Fault_Record_Payload]]
  }
