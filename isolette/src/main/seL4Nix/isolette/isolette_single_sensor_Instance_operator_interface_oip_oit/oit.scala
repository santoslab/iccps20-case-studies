...
package isolette.isolette_single_sensor_Instance_operator_interface_oip_oit
...
import isolette._
import isolette.Isolette.operator_interface_thread_impl_seL4Nix

object oit extends App {

  val oitBridge : isolette.Isolette.operator_interface_thread_impl_Bridge = {
  ...
  def receiveInput(eventPortIds: ISZ[Art.PortId], dataPortIds: ISZ[Art.PortId]): Unit = {
    ...
    regulator_status_port = operator_interface_thread_impl_seL4Nix.regulator_status_Receive()

    monitor_status_port = operator_interface_thread_impl_seL4Nix.monitor_status_Receive()

    display_temperature_port = operator_interface_thread_impl_seL4Nix.display_temperature_Receive()

    alarm_control_port = operator_interface_thread_impl_seL4Nix.alarm_control_Receive()
  }
  ...
  def sendOutput(eventPortIds: ISZ[Art.PortId], dataPortIds: ISZ[Art.PortId]): Unit = {
    ...
    if(lower_desired_tempWstatus_port.nonEmpty) {
      operator_interface_thread_impl_seL4Nix.lower_desired_tempWstatus_Send(lower_desired_tempWstatus_port.get)
      lower_desired_tempWstatus_port = noData
    }

    if(upper_desired_tempWstatus_port.nonEmpty) {
      operator_interface_thread_impl_seL4Nix.upper_desired_tempWstatus_Send(upper_desired_tempWstatus_port.get)
      upper_desired_tempWstatus_port = noData
    }

    if(lower_alarm_tempWstatus_port.nonEmpty) {
      operator_interface_thread_impl_seL4Nix.lower_alarm_tempWstatus_Send(lower_alarm_tempWstatus_port.get)
      lower_alarm_tempWstatus_port = noData
    }

    if(upper_alarm_tempWstatus_port.nonEmpty) {
      operator_interface_thread_impl_seL4Nix.upper_alarm_tempWstatus_Send(upper_alarm_tempWstatus_port.get)
