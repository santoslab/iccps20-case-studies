...
class Flow_Rate_Checker_imp_safety_alarm_process_flow_rate_Test extends BridgeTestSuite[Flow_Rate_Checker_imp_safety_alarm_process_flow_rate_Bridge](Arch.pca_imp_Instance_safety_alarm_process_flow_rate) {
  ...
  // setter for in DataPort
  def put_Upstream_Flow_Rate(value : PCA_Types.Flow_Rate): Unit = {
    ArtNative_Ext.insertInPortValue(bridge.operational_api.Upstream_Flow_Rate_Id, PCA_Types.Flow_Rate_Payload(value))
  }

  // setter for in DataPort
  def put_Downstream_Flow_Rate(value : PCA_Types.Flow_Rate): Unit = {
    ArtNative_Ext.insertInPortValue(bridge.operational_api.Downstream_Flow_Rate_Id, PCA_Types.Flow_Rate_Payload(value))
  }
