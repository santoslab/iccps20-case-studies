...
Unit building_control_gen_mixed_BuildingControl_TempSensor_i_seL4Nix_currentTemp_Send(
  STACK_FRAME
  art_DataContent d) {
  DeclNewStackFrame(caller, "sb_TempSensor_i.c", "", "building_control_gen_mixed_BuildingControl_TempSensor_i_seL4Nix_currentTemp_Send", 0);

  sb_currentTemp_write(d);
}

// send tempChanged: Out EventPort
Unit building_control_gen_mixed_BuildingControl_TempSensor_i_seL4Nix_tempChanged_Send(
  STACK_FRAME
  art_DataContent d) {
  DeclNewStackFrame(caller, "sb_TempSensor_i.c", "", "building_control_gen_mixed_BuildingControl_TempSensor_i_seL4Nix_tempChanged_Send", 0);
...
void pre_init(void) {
  ...
  printf("Entering pre-init of TempSensor_i\n");
  ...
  *sb_tempChanged_counter = 0;
  ...
  printf("Leaving pre-init of TempSensor_i\n");
}
