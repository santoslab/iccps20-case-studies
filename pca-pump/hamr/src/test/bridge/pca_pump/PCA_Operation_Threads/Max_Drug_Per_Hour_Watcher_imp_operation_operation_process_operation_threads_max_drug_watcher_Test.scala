...
class Max_Drug_Per_Hour_Watcher_imp_operation_operation_process_operation_threads_max_drug_watcher_Test extends BridgeTestSuite[Max_Drug_Per_Hour_Watcher_imp_operation_operation_process_operation_threads_max_drug_watcher_Bridge](Arch.pca_imp_Instance_operation_operation_process_operation_threads_max_drug_watcher) {
  ...
  // setter for in DataPort
  def put_Max_Drug_Per_Hour(value : PCA_Types.Drug_Volume): Unit = {
    ArtNative_Ext.insertInPortValue(bridge.operational_api.Max_Drug_Per_Hour_Id, PCA_Types.Drug_Volume_Payload(value))
  }
