...
package isolette.isolette_single_sensor_Instance_thermostat_regulate_temperature_manage_regulator_interface_mrit
...
import isolette._
import isolette.Regulate.Manage_Regulator_Interface_impl_seL4Nix

object mrit extends App {

  val mritBridge : isolette.Regulate.Manage_Regulator_Interface_impl_Bridge = {
  ...
  def receiveInput(eventPortIds: ISZ[Art.PortId], dataPortIds: ISZ[Art.PortId]): Unit = {
    ...
    current_tempWstatus_port = Manage_Regulator_Interface_impl_seL4Nix.current_tempWstatus_Receive()

    lower_desired_tempWstatus_port = Manage_Regulator_Interface_impl_seL4Nix.lower_desired_tempWstatus_Receive()

    upper_desired_tempWstatus_port = Manage_Regulator_Interface_impl_seL4Nix.upper_desired_tempWstatus_Receive()

    regulator_mode_port = Manage_Regulator_Interface_impl_seL4Nix.regulator_mode_Receive()
  }
  ...
  def sendOutput(eventPortIds: ISZ[Art.PortId], dataPortIds: ISZ[Art.PortId]): Unit = {
    ...
    if(upper_desired_temp_port.nonEmpty) {
      Manage_Regulator_Interface_impl_seL4Nix.upper_desired_temp_Send(upper_desired_temp_port.get)
      upper_desired_temp_port = noData
    }

    if(lower_desired_temp_port.nonEmpty) {
      Manage_Regulator_Interface_impl_seL4Nix.lower_desired_temp_Send(lower_desired_temp_port.get)
      lower_desired_temp_port = noData
    }

    if(displayed_temp_port.nonEmpty) {
      Manage_Regulator_Interface_impl_seL4Nix.displayed_temp_Send(displayed_temp_port.get)
      displayed_temp_port = noData
    }

    if(regulator_status_port.nonEmpty) {
      Manage_Regulator_Interface_impl_seL4Nix.regulator_status_Send(regulator_status_port.get)
      regulator_status_port = noData
    }

    if(interface_failure_port.nonEmpty) {
      Manage_Regulator_Interface_impl_seL4Nix.interface_failure_Send(interface_failure_port.get)
