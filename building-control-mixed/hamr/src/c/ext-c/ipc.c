...
Z building_control_mixed_SharedMemory_create(STACK_FRAME Z id) {
    unsigned int permission = 0666;
    unsigned int mask = IPC_CREAT;
...
Unit building_control_mixed_SharedMemory_receive(STACK_FRAME Z port, MBox2_1CBFC4 out) {
...
Unit building_control_mixed_SharedMemory_receiveAsync(STACK_FRAME Z port, MBox2_1029D1 out) {
    int sid = semget((key_t) port, 1, 0666);
...
Unit building_control_mixed_SharedMemory_send(STACK_FRAME Z destid, Z port, art_DataContent d) {
    int sid = semget((key_t) port, 1, 0666);
...
B building_control_mixed_SharedMemory_sendAsync(STACK_FRAME Z destid, Z port, art_DataContent d) {
    int sid = semget((key_t) port, 1, 0666);
...
Unit building_control_mixed_SharedMemory_remove(STACK_FRAME Z id) {
    semctl(semget((key_t) id, 1, 0666), 0, IPC_RMID);
    shmctl(shmget((key_t) id, sizeof(union Option_8E9F45), 0666), IPC_RMID, NULL);
}

Unit building_control_mixed_Process_sleep(STACK_FRAME Z n) {
