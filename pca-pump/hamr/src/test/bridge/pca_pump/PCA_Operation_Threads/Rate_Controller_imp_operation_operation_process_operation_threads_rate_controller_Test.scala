...
class Rate_Controller_imp_operation_operation_process_operation_threads_rate_controller_Test extends BridgeTestSuite[Rate_Controller_imp_operation_operation_process_operation_threads_rate_controller_Bridge](Arch.pca_imp_Instance_operation_operation_process_operation_threads_rate_controller) {
  ...
  def put_Square_Bolus_Rate(value : PCA_Types.Flow_Rate): Unit = {
    ArtNative_Ext.insertInPortValue(bridge.operational_api.Square_Bolus_Rate_Id, PCA_Types.Flow_Rate_Payload(value))
  }

  // setter for in DataPort
  def put_Patient_Bolus_Rate(value : PCA_Types.Flow_Rate): Unit = {
    ArtNative_Ext.insertInPortValue(bridge.operational_api.Patient_Bolus_Rate_Id, PCA_Types.Flow_Rate_Payload(value))
  }

  // setter for in DataPort
  def put_Basal_Rate(value : PCA_Types.Flow_Rate): Unit = {
    ArtNative_Ext.insertInPortValue(bridge.operational_api.Basal_Rate_Id, PCA_Types.Flow_Rate_Payload(value))
  }

  // setter for in DataPort
  def put_VTBI(value : PCA_Types.Drug_Volume): Unit = {
    ArtNative_Ext.insertInPortValue(bridge.operational_api.VTBI_Id, PCA_Types.Drug_Volume_Payload(value))
  }
  ...
  // setter for in EventDataPort
  def put_CP_Bolus_Duration(value : ICE_Types.Minute): Unit = {
    ArtNative_Ext.insertInPortValue(bridge.operational_api.CP_Bolus_Duration_Id, ICE_Types.Minute_Payload(value))
  }
  ...
  def get_Infusion_Flow_Rate(): Option[PCA_Types.Flow_Rate] = {
    val value: Option[PCA_Types.Flow_Rate] = get_Infusion_Flow_Rate_payload() match {
      case Some(PCA_Types.Flow_Rate_Payload(v)) => Some(v)
      case Some(v) => fail(s"Unexpected payload on port Infusion_Flow_Rate.  Expecting 'PCA_Types.Flow_Rate_Payload' but received ${v}")
  ...
  def get_Infusion_Flow_Rate_payload(): Option[PCA_Types.Flow_Rate_Payload] = {
    return ArtNative_Ext.observeOutPortValue(bridge.initialization_api.Infusion_Flow_Rate_Id).asInstanceOf[Option[PCA_Types.Flow_Rate_Payload]]
  }
