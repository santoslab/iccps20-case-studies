...
class Security_Thread_imp_operation_security_subsystem_security_process_security_thread_Test extends BridgeTestSuite[Security_Thread_imp_operation_security_subsystem_security_process_security_thread_Bridge](Arch.pca_imp_Instance_operation_security_subsystem_security_process_security_thread) {
  ...
  // getter for out DataPort
  def get_Prescription(): Option[PCA_Types.Prescription] = {
    val value: Option[PCA_Types.Prescription] = get_Prescription_payload() match {
      case Some(PCA_Types.Prescription_Payload(v)) => Some(v)
      case Some(v) => fail(s"Unexpected payload on port Prescription.  Expecting 'PCA_Types.Prescription_Payload' but received ${v}")
  ...
  // payload getter for out DataPort
  def get_Prescription_payload(): Option[PCA_Types.Prescription_Payload] = {
    return ArtNative_Ext.observeOutPortValue(bridge.initialization_api.Prescription_Id).asInstanceOf[Option[PCA_Types.Prescription_Payload]]
  }
