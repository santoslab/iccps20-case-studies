...
Z b_SharedMemory_create(STACK_FRAME Z id) {
    unsigned int permission = 0666;
    unsigned int mask = IPC_CREAT;
...
Unit b_SharedMemory_receive(STACK_FRAME Z port, MBox2_1CBFC4 out) {
...
Unit b_SharedMemory_receiveAsync(STACK_FRAME Z port, MBox2_1029D1 out) {
    int sid = semget((key_t) port, 1, 0666);
...
Unit b_SharedMemory_send(STACK_FRAME Z destid, Z port, art_DataContent d) {
    int sid = semget((key_t) port, 1, 0666);
...
B b_SharedMemory_sendAsync(STACK_FRAME Z destid, Z port, art_DataContent d) {
    int sid = semget((key_t) port, 1, 0666);
