...
class Patient_Bolus_Checker_imp_operation_operation_process_operation_threads_patient_bolus_checker_Test extends BridgeTestSuite[Patient_Bolus_Checker_imp_operation_operation_process_operation_threads_patient_bolus_checker_Bridge](Arch.pca_imp_Instance_operation_operation_process_operation_threads_patient_bolus_checker) {
  ...
  def put_Minimum_Time_Between_Bolus(value : ICE_Types.Minute): Unit = {
    ArtNative_Ext.insertInPortValue(bridge.operational_api.Minimum_Time_Between_Bolus_Id, ICE_Types.Minute_Payload(value))
  }
