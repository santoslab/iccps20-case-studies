...
class ui_thread_imp_operation_control_panel_ui_process_ui_thread_Test extends BridgeTestSuite[ui_thread_imp_operation_control_panel_ui_process_ui_thread_Bridge](Arch.pca_imp_Instance_operation_control_panel_ui_process_ui_thread) {
  ...
  def get_Bolus_Duration(): Option[ICE_Types.Minute] = {
    val value: Option[ICE_Types.Minute] = get_Bolus_Duration_payload() match {
      case Some(ICE_Types.Minute_Payload(v)) => Some(v)
      case Some(v) => fail(s"Unexpected payload on port Bolus_Duration.  Expecting 'ICE_Types.Minute_Payload' but received ${v}")
  ...
  def get_Bolus_Duration_payload(): Option[ICE_Types.Minute_Payload] = {
    return ArtNative_Ext.observeOutPortValue(bridge.initialization_api.Bolus_Duration_Id).asInstanceOf[Option[ICE_Types.Minute_Payload]]
  }
