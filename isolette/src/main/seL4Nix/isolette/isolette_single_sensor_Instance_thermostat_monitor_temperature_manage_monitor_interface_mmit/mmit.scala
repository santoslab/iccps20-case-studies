...
package isolette.isolette_single_sensor_Instance_thermostat_monitor_temperature_manage_monitor_interface_mmit
...
import isolette._
import isolette.Monitor.Manage_Monitor_Interface_impl_seL4Nix

object mmit extends App {

  val mmitBridge : isolette.Monitor.Manage_Monitor_Interface_impl_Bridge = {
  ...
  def receiveInput(eventPortIds: ISZ[Art.PortId], dataPortIds: ISZ[Art.PortId]): Unit = {
    ...
    upper_alarm_tempWstatus_port = Manage_Monitor_Interface_impl_seL4Nix.upper_alarm_tempWstatus_Receive()

    lower_alarm_tempWstatus_port = Manage_Monitor_Interface_impl_seL4Nix.lower_alarm_tempWstatus_Receive()

    current_tempWstatus_port = Manage_Monitor_Interface_impl_seL4Nix.current_tempWstatus_Receive()

    monitor_mode_port = Manage_Monitor_Interface_impl_seL4Nix.monitor_mode_Receive()
  }
  ...
  def sendOutput(eventPortIds: ISZ[Art.PortId], dataPortIds: ISZ[Art.PortId]): Unit = {
    ...
    if(upper_alarm_temp_port.nonEmpty) {
      Manage_Monitor_Interface_impl_seL4Nix.upper_alarm_temp_Send(upper_alarm_temp_port.get)
      upper_alarm_temp_port = noData
    }

    if(lower_alarm_temp_port.nonEmpty) {
      Manage_Monitor_Interface_impl_seL4Nix.lower_alarm_temp_Send(lower_alarm_temp_port.get)
      lower_alarm_temp_port = noData
    }

    if(monitor_status_port.nonEmpty) {
      Manage_Monitor_Interface_impl_seL4Nix.monitor_status_Send(monitor_status_port.get)
      monitor_status_port = noData
    }

    if(interface_failure_port.nonEmpty) {
      Manage_Monitor_Interface_impl_seL4Nix.interface_failure_Send(interface_failure_port.get)
