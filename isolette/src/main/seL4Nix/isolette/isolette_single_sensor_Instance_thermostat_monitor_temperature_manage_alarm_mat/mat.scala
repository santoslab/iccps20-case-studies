...
package isolette.isolette_single_sensor_Instance_thermostat_monitor_temperature_manage_alarm_mat
...
import isolette._
import isolette.Monitor.Manage_Alarm_impl_seL4Nix

object mat extends App {

  val matBridge : isolette.Monitor.Manage_Alarm_impl_Bridge = {
  ...
  def receiveInput(eventPortIds: ISZ[Art.PortId], dataPortIds: ISZ[Art.PortId]): Unit = {
    ...
    current_tempWstatus_port = Manage_Alarm_impl_seL4Nix.current_tempWstatus_Receive()

    lower_alarm_temp_port = Manage_Alarm_impl_seL4Nix.lower_alarm_temp_Receive()

    upper_alarm_temp_port = Manage_Alarm_impl_seL4Nix.upper_alarm_temp_Receive()

    monitor_mode_port = Manage_Alarm_impl_seL4Nix.monitor_mode_Receive()
  }
  ...
  def sendOutput(eventPortIds: ISZ[Art.PortId], dataPortIds: ISZ[Art.PortId]): Unit = {
    ...
    if(alarm_control_port.nonEmpty) {
      Manage_Alarm_impl_seL4Nix.alarm_control_Send(alarm_control_port.get)
